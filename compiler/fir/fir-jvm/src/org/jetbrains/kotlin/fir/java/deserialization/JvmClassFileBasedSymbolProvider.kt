...
/**
 * This symbol provider loads JVM classes, reading extra info from Kotlin [`@Metadata`][Metadata]
 * annotations if present. Use it for library and incremental compilation sessions. For source
 * sessions use [org.jetbrains.kotlin.fir.java.JavaSymbolProvider], as Kotlin classes should be
 * parsed first.
 *
 * 이 심볼 제공자는 JVM 클래스를 로드하며, 존재할 경우 Kotlin의 [@Metadata][Metadata] 애노테이션에서
 * 추가 정보를 읽습니다. 라이브러리나 증분 컴파일 세션에서 사용하도록 설계되었습니다. 소스 세션의 경우
 * Kotlin 클래스가 먼저 파싱되어야 하므로 [org.jetbrains.kotlin.fir.java.JavaSymbolProvider]를 사용하는
 * 것이 좋습니다.
 */
